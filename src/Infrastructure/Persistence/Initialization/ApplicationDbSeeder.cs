using TD.WebApi.Infrastructure.Identity;
...
internal class ApplicationDbSeeder
{
    ...
    private async Task SeedRolesAsync(ApplicationDbContext dbContext)
    {
        foreach (string roleName in TDRoles.DefaultRoles)
        {
            ...
            // Assign permissions
            if (roleName == TDRoles.Basic)
            {
                await AssignPermissionsToRoleAsync(dbContext, TDPermissions.Basic, role);
            }
            else if (roleName == TDRoles.Admin)
            {
